...
class FluxAttentionEraser(nn.Module):
    """Extracts cross attention maps for a specific token from Flux model."""
    
    def __init__(self, height: int = 64, inner_dim=None, dtype=torch.bfloat16, layer_ind: int = 0, scale_eraser_key_inference=1.0, mode=Mode.TRAIN, config=None):
        super().__init__()
    ...
    def __call__(
        ...
        **kwargs
        ...
        # self.spiderman_val = (torch.load(os.path.join('spiderman_val', f'{self.layer_ind}.pt'), map_location=self.value_addition.device).mean(dim=2)).view(1, 1, -1)
        ...
        if self.mode in {Mode.TRAIN}: 
            ...
            if len(self.target_token_idx_lst) > 1:
                print(f"#########Note: the batch size {batch_size} >1. Adjust FluxAttentionEraser call func")
        ...
        # - INJECT ERASER KEY/VALUE RIGHT BEFORE ATTENTION, only in train or inference mode -
            ...
            eraser_key_batch = self.key_addition.expand(batch_size, -1, -1)
            ...
            if (self.scale_eraser_key_inference is not None) and (self.mode in {Mode.INFERENCE, Mode.EVAL}):
                eraser_key_batch = eraser_key_batch * self.scale_eraser_key_inference
            ...
            if self.mode in {Mode.INFERENCE, Mode.TRAIN_SKIP, Mode.TRAIN_NO_TARGET}:
                ...
                if self.mode == Mode.INFERENCE and self.config is not None and self.config["save_keys_in_inference"]:
                    ...
                    if not os.path.isfile(ckpt_path): # during inference only save the first
                        ...
                        torch.save(self.key_addition.detach().cpu(), ckpt_path)
